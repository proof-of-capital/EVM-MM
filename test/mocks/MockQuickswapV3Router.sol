...
import {IQuickswapV3Router} from "../../src/inerfaces/IQuickswapV3Router.sol";
import {IERC20} from "@openzeppelin/contracts/token/ERC20/IERC20.sol";

// Mock QuickswapV3 Router (Algebra)
contract MockQuickswapV3Router is IQuickswapV3Router {
    ...
    function exactInputSingleSupportingFeeOnTransferTokens(ExactInputSingleParams calldata params)
        ...
        returns (uint256 amountOut)
    {
        // Same as exactInputSingle but handles fee on transfer tokens
        uint256 rate = swapRates[params.tokenIn][params.tokenOut];
        if (rate == 0) {
            rate = 1e18; // Default 1:1 if not set
        }
    ...
    function algebraSwapCallback(int256, int256, bytes calldata) external pure override {
