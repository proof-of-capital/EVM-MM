...
contract RebalanceV2UniswapV3Test is Test {
    ...
    // ============ SwapRouterBase Tests ============

    function test_rebalanceLPtoPOC_SwapRouterBase_Success() public {
        // Deploy SwapRouterBase router
        MockSwapRouterBase swapRouterBase = new MockSwapRouterBase();
        ...
        swapParamsArray[0] = SwapParams({
            ...
            routerAddress: address(swapRouterBase),
            path: new address[](0), // Not used for V3
            data: path1,
            amountOutMinimum: 900e18
        });
        ...
        // Setup swap rate to return more collateral (1.1:1)
        swapRouterBase.setSwapRate(address(launchToken), address(collateral1), 11e17); // 1.1:1

        // Mint enough collateral to SwapRouterBase router for 1.1x return
        // Initial balance in rebalanceV2 is 1e24, so router needs at least 1.1e24 collateral
        collateral1.mint(address(swapRouterBase), 2e24);

        // Prepare POC buy params
        // Strategy: swap returns 1.1x collateral, then buy returns 1.1x launchToken
        // After swap: 1e24 launchToken * 1.1 = 1.1e24 collateral
        // Calculate exact amount: initialLaunchToken * 1.1 (swap rate)
        // Using: (amount * 110) / 100 for 1.1x multiplier
        uint256 collateralReceived = (initialLaunchToken * 110) / 100; // 1.1x
        POCBuyParams[] memory pocBuyParamsArray = new POCBuyParams[](1);
        pocBuyParamsArray[0] = POCBuyParams({
            pocContract: address(poc1),
            collateral: address(collateral1),
            collateralAmount: collateralReceived // Use all collateral received from swap
        });

        // Mint launch tokens to POC contract for buy operations
        // We buy with collateralReceived collateral, MockPOC returns 1.1x launch tokens
        launchToken.mint(address(poc1), 2e24);

        // Approve SwapRouterBase router to spend tokens from rebalance contract
        // Note: allowance for collateral1 to poc1 is already set in setUp()
        AllowanceParams[] memory allowances = new AllowanceParams[](1);
        allowances[0] =
            AllowanceParams({token: address(launchToken), spender: address(swapRouterBase), amount: type(uint256).max});
        rebalanceV2.increaseAllowanceForSpenders(allowances);
        ...
        // Verify POC interactions
        assertEq(poc1.tokensReceivedOnBuy(), collateralReceived, "POC1 should receive correct amount");
    ...
    function test_rebalancePOCtoLP_SwapRouterBase_Success() public {
        // Deploy SwapRouterBase router
        MockSwapRouterBase swapRouterBase = new MockSwapRouterBase();
        ...
        // Setup swap rate for profit (collateral -> launchToken with profit)
        swapRouterBase.setSwapRate(address(collateral3), address(launchToken), 11e17); // 1.1:1
        swapRouterBase.setSwapRate(address(collateral4), address(launchToken), 11e17); // 1.1:1

        // Mint launch tokens to SwapRouterBase router for swaps
        // After selling: 1.5e21 * 1.1 = 1.65e21 collateral each, total 3.3e21
        // After swap: 3.3e21 * 1.1 = 3.63e21 launchToken each
        launchToken.mint(address(swapRouterBase), 5e24);
    ...
    function test_rebalancePOCtoPOC_SwapRouterBase_Success() public {
        // Deploy SwapRouterBase router
        MockSwapRouterBase swapRouterBase = new MockSwapRouterBase();
        ...
        // Setup swap rates (1:1 for simplicity)
        swapRouterBase.setSwapRate(address(collateral3), address(collateral1), 1e18);
        swapRouterBase.setSwapRate(address(collateral4), address(collateral2), 1e18);

        // Mint collateral tokens to SwapRouterBase router for swaps
        // After selling: 1.5e21 * 1.1 = 1.65e21 collateral each
        // After swap: 1.65e21 collateral -> 1.65e21 target collateral (1:1)
        collateral1.mint(address(swapRouterBase), 2e24);
        collateral2.mint(address(swapRouterBase), 2e24);
